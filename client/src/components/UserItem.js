import React from "react";
import { useAppContext } from "../context/AppContext";
import "./components.css";

export const UserItem = ({ userItem }) => {
  const {setSelectedUser,getMessages,user} = useAppContext();
  var selectedUser = userItem.user;
  const handleClick = () =>{
    var usersCol = document.getElementById("usersCol");
    var chatCol = document.getElementById("chatCol");
    usersCol.classList.toggle('usersCol-trans');
    chatCol.classList.toggle('chatCol-trans');
    setSelectedUser(userItem);
    // getMessages(user,userItem.user);
  }
  if(userItem.profPic==''){
    userItem.profPic = "data:image/png;base64,";
  }
  const getMessageData = (messages) => {
    userItem.user.messages = messages;
    return (
      <div className="d-flex flex-row align-items-center">
        {messages.length==0 && userItem.lastMessage && <p style={{color:'grey'}}>{userItem.lastMessage}</p>}
        {messages.length!=0 && <p className="highlight-text">{messages.length}</p>}
        <p className="m-0">{messages[messages.length-1]}</p>
      </div>
    )
  }
  return (
    <div key={Math.random()*1000} className="card p-2" style={{cursor:'pointer',borderRadius:'0px'}} onClick={handleClick}>
      <div className="d-flex flex-row justify-content-start">
        <div className="d-flex flex-column">
          <img src={userItem.profPic} style={{objectFit:'cover',borderRadius:'100px',border:'2px solid black'}} height='60px' width='60px'/>
        </div>
        <div className="d-flex flex-column justify-content-center" style={{paddingLeft:'20px'}}>
          <div style={{ fontWeight: "bold", letterSpacing: "2px" }}>
            {selectedUser.name}
          </div>
          
          {userItem.messages?<></>:<p style={{ fontSize: "14px",marginBottom:'0px' }}>{selectedUser.email}</p>}
          {userItem.messages?getMessageData(userItem.messages):<></>}
          {/* {userItem.messages?Object.values(userItem.messages).map((item)=>{
            return <p key={Math.random()*1000}>{item}</p>
          }):<></>} */}
        </div>
      </div>
    </div>
  );
};
