...
const reducer = (state, action) => {
  ...
  if (action.type === DISPLAY_ALERT) {
    ...
    return {
      ...
      showAlert: true,
      alertType: "success",
      alertMessage: ["User being Registered. Please wait..."],
    };
    ...
    return {
      ...
      showAlert: true,
      alertType: "success",
      alertMessage: ["User Registered. Redirecting..."],
    ...
    return {
      ...
      showAlert: true,
      alertType: "success",
      alertMessage: ["Please wait while User being logged In..."],
    };
    ...
    return {
      ...
      showAlert: true,
      alertType: "success",
      alertMessage: ["User Logged In. Redirecting..."],
      user: action.payload.user,
      token: action.payload.token,
    };
  ...
  } else if (action.type === GET_TODOS) {
    return {
      ...state,
      todos: action.todos,
      isLoading: false,
    }
  } else if (action.type === SET_TODOITEM) {
    return {
      ...state,
      todoItem: action.payload
    }
  } else if (action.type === SET_PROFILE_PIC) {
    return {
      ...state,
      profileImage: action.payload.img,
      isLoading: false
    }
    ...
    if (action.payload.user) {
      var selectedUser = action.payload.user;
      var user = state.user;
      var chatId = ""
      if (user._id > selectedUser._id) {
        chatId = user._id + "_" + selectedUser.user._id;
      } else {
        chatId = selectedUser.user._id + "_" + user._id;
      }
      return {
        ...state,
        chatMessages: [],
  ...
  } else if (action.type === SET_SCROLL_TO_BOTTOM) {
    return {
      ...state,
      scrollToBottom: false
    }
  } else if( action.type ===  SET_PARTICIPANTS_ONLINE){
    return {
      ...state,
      participantsOnline: action.payload.isOnline
    }
  } else if( action.type === ADD_FILE_DICT){
    var dict = state.files
    dict[action.payload.fileItem.fileId] = action.payload.fileItem
    return{
      ...state,
      files: dict
    }
